...
module.exports = (profile) => {
  ...
  if (profile.honors) {
    profile.honors = profile.honors.map(({ name, date, issuer, description }) => ({
  ...
  if (profile.projects) {
    profile.projects = profile.projects.map(({ name, date, description, link }) => ({
  ...
  if (profile.organizations) {
    profile.organizations = profile.organizations.map(({ name, date, position, description }) => ({
  ...
  if (profile.patents) {
    profile.patents = profile.patents.map(({ name, date, issuer, description, link }) => ({
  ...
  if (profile.publications) {
    profile.publications = profile.publications.map(({ name, date, publisher, description, link }) => ({
  ...
  if (profile.testScores) {
    profile.testScores = profile.testScores.map(({ name, date, score, description }) => ({
